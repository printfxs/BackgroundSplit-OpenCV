 ...
 *
 * Summary:
 *   Update Background Model and Sample Library.
 *
 * Arguments:
 *   Mat img - source image
 *
    ...
    for(int i = 0; i < img.rows; i++)
	{
        for(int j = 0; j < img.cols; j++)
		{
            ...
            */
			...
			int dist;
            for(count = 0, matches = 0; matches < num_min_matches && count < num_samples; count++)
            {
                dist = abs(samples[i][j][count] - img.at<uchar>(i, j));
                if (dist < radius)
                    matches++;
            }
             ...
             *   - Update the times of Foreground Statistic
             *   - Update Foreground Model
             *   - Update model sample library of this pixel probably
             *   - Update model sample library of this pixel's neighborhood pixel probably
            =====================================================================
            */
            if (matches >= num_min_matches)
			{
                ...
                // This pixel is already regarded as Background Pixel, then it has possibility of 1/φ to update its model sample's value.
                ...
                // At the same time, it has possibility of 1/φ to update its neighborhood point's sample value.
...
>>>>>>> origin/master
             ...
             *   Then it needs to be done:
             *   - Update the times of Foreground Statistic
             *   - Judge and Update Foreground Model
            =====================================================================
            */
            else
			{
                ...
                // if this pixel is regarded as foreground for more than 50 times, then we regard this static area as dynamic area by mistake, and update this pixel as background one.
